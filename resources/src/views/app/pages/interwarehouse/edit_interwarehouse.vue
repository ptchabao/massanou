<template>
  <div class="main-content">
    <breadcumb :page="$t('EditInterWarehouseRequest')" :folder="$t('InterWarehouseRequests')"/>
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>

    <validation-observer ref="Edit_InterWarehouse" v-if="!isLoading">
                  ...
                  <validation-provider name="Requester Warehouse" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('RequesterWarehouse') + ' ' + '*'">
                      <v-select
                        ...
                        @input="Selected_Requester_Warehouse"
...
export default {
  metaInfo: {
    title: "Edit Inter-Warehouse Request"
  },
  data() {
    return {
      focused: false,
  ...
  methods: {
    ...
    Submit_Request() {
      this.$refs.Edit_InterWarehouse.validate().then(success => {
        if (!success) {
          this.makeToast(
            "danger",
    ...
    search(){
      ...
      if (this.iw_request.requester_warehouse_id != "" &&  this.iw_request.requester_warehouse_id != null) {
    ...
    Selected_Requester_Warehouse(value) {
      this.search_input= '';
      this.product_filter = [];
    ...
    Update_Request() {
      ...
      NProgress.set(0.1);
      axios
        .put("interwarehouse_requests/" + this.iw_request.id, {
    ...
    GetElements() {
      ...
      axios
        .get("interwarehouse_requests/" + id + "/edit")
        .then(response => {
          ...
          this.Get_Products_By_Warehouse(this.iw_request.requester_warehouse_id);
