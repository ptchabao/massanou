...
class AutoUpdateController extends Controller
{
    public function oneClickUpdate(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                if ($migrationsDone) {
                    ...
                    if ($sql && File::exists($sql)) {
                        ...
                        if (!$restored) {
                            try { Artisan::call('migrate:rollback', ['--force' => true]); } catch (\Throwable $mr) {}
                        }
                    } else {
                        try { Artisan::call('migrate:rollback', ['--force' => true]); } catch (\Throwable $mr) {}
                    }
            ...
            try { Artisan::call('up'); } catch (\Throwable $ignored) {}
            $this->releaseUpdateLock();
    ...
    {
        $items = array_values(array_filter(scandir($path), function ($i) { return !in_array($i, ['.', '..']); }));
    ...
    private function mergeIntoApplication(string $source, string $destination, array $excludeRelativePaths): void
    {
        ...
        foreach ($iterator as $item) {
            ...
            if ($this->containsPathTraversal($relPath)) { continue; }
    ...
    private function isExcluded(string $relativePath, array $exclusions): bool
    {
        foreach ($exclusions as $exclude) {
            ...
            if ($exclude === '') continue;
            if ($relativePath === $exclude) return true;
            if (strpos($relativePath, $exclude . DIRECTORY_SEPARATOR) === 0) return true;
        }
        return false;
    }
    ...
    private function cleanBaseExcept(string $base, array $excludeRelativePaths): void
    {
        ...
        $items = array_values(array_filter(scandir($base), function ($i) { return !in_array($i, ['.', '..']); }));
    ...
    {
        if (strpos($rel, '..') !== false) return true;
        $relNorm = str_replace('\\', '/', $rel);
        if (isset($relNorm[0]) && ($relNorm[0] === '/' || $relNorm[0] === '\\')) return true;
        if (preg_match('/^[a-zA-Z]:\//', $relNorm)) return true;
        return false;
    }
    ...
    private function zipDoesNotContainForbidden(string $zipPath): bool
    {
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if ($lower === '.env' || strpos($lower, '/.env') !== false) { $zip->close(); return false; }
            if (strpos($lower, 'storage/') === 0) { $zip->close(); return false; }
            if (strpos($lower, 'public/images') === 0) { $zip->close(); return false; }
            if (strpos($lower, 'modules/') === 0 || $lower === 'modules_statuses.json') { $zip->close(); return false; }
        }
        $zip->close();
        return true;
    }
    ...
    {
        $lock = storage_path('app/updater.lock');
        try { if (File::exists($lock)) File::delete($lock); } catch (\Throwable $e) {}
    }
    ...
    private function createApplicationBackupZip(): string
    {
        ...
        $normalize = function (string $path): string { return str_replace(['\\', '/'], DIRECTORY_SEPARATOR, $path); };
        $isExcluded = function (string $relative) use ($excludePaths, $normalize): bool {
            ...
            foreach ($excludePaths as $ex) {
                ...
                if (strpos($relative, $ex . DIRECTORY_SEPARATOR) === 0) return true;
            }
            return false;
        };
        ...
        foreach ($iterator as $item) {
            ...
            if ($relLower === 'storage/app/updater.lock' || $relLower === 'storage/app/update_progress.json') continue;
            if ($item->isDir()) {
                // Ensure directory entry exists in zip for empty directories
                if ($rel !== '') { $zip->addEmptyDir($rel); }
            } else {
    ...
    private function runFinalizationTasks(): void
    {
        ...
        foreach ($permissions as $permission_slug) { Permission::firstOrCreate(['name' => $permission_slug]); }
        ...
        if ($nexmoGateway) { $nexmoGateway->delete(); }
        ...
        \App\Models\Product::where('name', 'REGEXP', '<|>')
            ->chunkById(200, function ($products) {
                foreach ($products as $p) {
                    ...
                    if ($clean !== $old) { $p->name = $clean; $p->save(); }
                }
            });
    }
    ...
    private function updateProgress(string $step, int $percent): void
    {
        ...
        try { File::put(storage_path('app/update_progress.json'), json_encode($payload)); } catch (\Throwable $e) {}
    }
    ...
    private function appendHistory(string $status, string $version): void
    {
        ...
        $hist = [];
        try { if (File::exists($file)) { $hist = json_decode(File::get($file), true) ?: []; } } catch (\Throwable $e) {}
        $hist[] = ['status' => $status, 'version' => $version, 'time' => now()->toDateTimeString()];
        try { File::put($file, json_encode($hist)); } catch (\Throwable $e) {}
    }
    ...
    public function preflight(): \Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            if ($json !== false) { $net['ok'] = true; $net['size'] = strlen($json); }
        } catch (\Throwable $e) { $net['error'] = $e->getMessage(); }
        // Disk space
    ...
    {
        if (!$p) return false;
        try { return file_exists($p); } catch (\Throwable $e) { return false; }
    }
