...
class ReportController extends BaseController
{
    ...
    public function Warhouse_Count_Stock(Request $request)
    {
        ...
        $count_items  = $stock_count->pluck('value')->map(function($v){ return (float)$v; })->values();
        $count_qty    = $stock_count->pluck('value1')->map(function($v){ return (float)$v; })->values();

        $value_labels = $stock_value->pluck('name')->values();
        $value_price  = $stock_value->pluck('price')->map(function($v){ return (float)$v; })->values();
        $value_cost   = $stock_value->pluck('cost')->map(function($v){ return (float)$v; })->values();
    ...
    public function cash_flow_report(Request $request)
    {
        ...
        if ($groupBy === 'method') {
            $q = PaymentSale::query()
                ...
                ->groupBy('g')
                ->get();
            foreach ($q as $row) { $addAmount($groupLabel('', $row->g), 'inflow', (float) $row->total); }
        } else {
            $q = PaymentSale::query()
                ->leftJoin('sales', 'payment_sales.sale_id', '=', 'sales.id')
        ...
        if ($groupBy === 'account') {
            ...
            $depositTotal = Deposit::query()
                ...
                ->sum('amount');
            if ($depositTotal) { $addAmount('Deposit', 'inflow', (float) $depositTotal); }
        }
        ...
        if ($groupBy === 'method') {
            $q = PaymentPurchase::query()
                ...
                ->groupBy('g')
                ->get();
            foreach ($q as $row) { $addAmount($groupLabel('', $row->g), 'outflow', (float) $row->total); }
        } else {
            $q = PaymentPurchase::query()
                ->leftJoin('purchases', 'payment_purchases.purchase_id', '=', 'purchases.id')
        ...
        $psByDate = PaymentSale::query()
            ...
            ->groupBy('d')->get();
        foreach ($psByDate as $r) { $inflowByDate[$r->d] = ($inflowByDate[$r->d] ?? 0) + (float) $r->t; }
        ...
        $depByDate = Deposit::query()
            ...
            ->groupBy('d')->get();
        foreach ($depByDate as $r) { $inflowByDate[$r->d] = ($inflowByDate[$r->d] ?? 0) + (float) $r->t; }
        ...
        $ppByDate = PaymentPurchase::query()
            ...
            ->groupBy('d')->get();
        foreach ($ppByDate as $r) { $outflowByDate[$r->d] = ($outflowByDate[$r->d] ?? 0) + (float) $r->t; }
        ...
        $expByDate = Expense::query()
            ...
            ->select(DB::raw('date as d'), DB::raw('SUM(amount) as t'))
            ->groupBy('d')->get();
        foreach ($expByDate as $r) { $outflowByDate[$r->d] = ($outflowByDate[$r->d] ?? 0) + (float) $r->t; }
    ...
    public function customerLoyaltyPoints(Request $request)
    {
        ...
        try { $this->authorizeForUser($request->user('api'), 'customer_loyalty_points_report', \App\Models\Sale::class); } catch (\Throwable $e) { /* fallback allow if no policy */ }
        ...
        if (!in_array($order, $allowed, true)) $order = 'earned_points';
