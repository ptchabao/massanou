...
class JournalEntriesController extends Controller
{
	public function index(Request $request)
	{
		...
		if (!Schema::hasTable('acc_journal_entries')) { return response()->json(['data' => [], 'total' => 0]); }
		$q = JournalEntry::query()->orderByDesc('date')->orderByDesc('id');
		if ($request->filled('status')) { $q->where('status', $request->get('status')); }
		if ($request->filled('reference_type')) { $q->where('reference_type', $request->get('reference_type')); }
		if ($request->filled('from')) { $q->where('date', '>=', $request->get('from')); }
		if ($request->filled('to')) { $q->where('date', '<=', $request->get('to')); }
	...
	public function post(Request $request , int $id)
	{
		...
		if ($entry->status === 'posted') { return response()->json($entry); }
