...
class ReportsController extends Controller
{
	public function trialBalance(Request $request)
	{
		...
		if (!Schema::hasTable('acc_journal_entry_lines')) { return response()->json(['data' => [], 'totalRows' => 0]); }
		...
		if ($from) { $base->where('j.date', '>=', $from); }
		if ($to) { $base->where('j.date', '<=', $to); }
		if ($request->filled('type')) { $base->where('c.type', $request->get('type')); }
		if ($search !== '') { $base->where(function ($q) use ($search) { $q->where('c.code', 'like', "%$search%")->orWhere('c.name', 'like', "%$search%"); }); }
	...
	public function profitAndLoss(Request $request)
	{
		...
		if (!Schema::hasTable('acc_journal_entry_lines')) { return response()->json(['data' => [], 'totalRows' => 0, 'summary' => ['income' => 0, 'expense' => 0, 'net_profit' => 0]]); }
		$from = $request->get('from');
		$to = $request->get('to');
		$search = trim((string) $request->get('search', ''));
		...
		if ($from) { $base->where('j.date', '>=', $from); }
		if ($to) { $base->where('j.date', '<=', $to); }
		if ($typeFilter) { $base->where('c.type', $typeFilter); }
		if ($search !== '') {
	...
	public function balanceSheet(Request $request)
	{
		...
		if (!Schema::hasTable('acc_journal_entry_lines')) { return response()->json(['data' => []]); }
