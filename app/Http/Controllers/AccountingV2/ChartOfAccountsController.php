...
class ChartOfAccountsController extends Controller
{
	public function index(Request $request)
	{
		...
		if (!Schema::hasTable('acc_chart_of_accounts')) { return response()->json(['data' => [], 'totalRows' => 0]); }
		$query = ChartOfAccount::query();
		// Filters
		if ($request->filled('type')) { $query->where('type', $request->get('type')); }
		if ($request->has('active')) { $query->where('is_active', (int) $request->get('active') === 1); }
		...
		if ($perPage === -1) { $perPage = $totalRows > 0 ? $totalRows : 10; }
